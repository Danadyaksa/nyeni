  ...
  Future<void> _loadStats() async {
    ...
    try {
      final results = await Future.wait([
        ...
        _adminService.getAllEventsAdmin(),
        _adminService.getRevenue(),
      ]);

      final tickets = results[0] as List<dynamic>;
      final events = results[1] as List<dynamic>;
      final revenue = results[2] as Map<String, dynamic>;

      setState(() {
        _pendingCount = tickets.where((t) => t['status'] == 'PENDING').length;
        _activeCount = tickets.where((t) => t['status'] == 'ACTIVE').length;
        _totalEvents = events.length;
        _totalRevenue = (revenue['total_revenue'] ?? 0) as int;
      });
    } catch (e) {
