...
class _AdminPaymentScreenState extends State<AdminPaymentScreen>
  ...
  Future<void> _loadTickets() async {
    ...
    final tickets = await _adminService.getAllTickets();
    if (mounted) {
      setState(() {
        _allTickets = tickets;
        _isLoading = false;
  ...
  Future<void> _handleAccept(String ticketId, String eventName, int ticketCount) async {
    ...
    final success = await _adminService.acceptTicket(ticketId);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success ? '✅ $ticketCount tiket berhasil diaktifkan!' : '❌ Gagal mengaktifkan tiket'),
  ...
  Future<void> _handleDecline(String ticketId, String eventName, int ticketCount) async {
    ...
    final success = await _adminService.declineTicket(ticketId);
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(success ? '🚫 $ticketCount tiket berhasil ditolak' : '❌ Gagal menolak tiket'),
